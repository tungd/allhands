...
let with_current_connection (conn : connection) f =
...
let mark_stream_writable (stream : stream) =
  ...
  stream.conn.last_active <- Unix.gettimeofday ();
  signal_wakeup stream.server

let respond_stream ?(status=`OK) ?(headers=[]) reqd =
  let server =
    match Atomic.get active_server with
    | Some server -> server
    | None -> failwith "HTTP streaming response requires an active server"
  in
  let conn =
    match !current_connection with
    | Some conn -> conn
    | None -> failwith "HTTP streaming response requires an active connection"
  in
  let headers =
    if has_ci_header "server" headers then headers
    else ("server", "OClaw/1.0") :: headers
  ...
  mark_stream_writable stream;
  stream

let write_stream stream text =
  Mutex.lock stream.mutex;
  try
    if stream.closed || Body.Writer.is_closed stream.body then
      stream.closed <- true
    else begin
      Body.Writer.write_string stream.body text;
      Body.Writer.flush_with_reason stream.body (function
        | `Written -> ()
...
let close_stream stream =
  Mutex.lock stream.mutex;
  try
    if not stream.closed then begin
      stream.closed <- true;
      if not (Body.Writer.is_closed stream.body) then Body.Writer.close stream.body;
      mark_stream_writable stream
    end;
    Mutex.unlock stream.mutex
  with exn ->
...
let remove_pending_jobs_for_conn (server : t) (conn : connection) =
...
let handle_readable (server : t) (conn : connection) =
...
let handle_writable (server : t) (conn : connection) =
  let rec loop () =
...
let close_idle_connections (server : t) now =
  let to_close = ref [] in
...
let process_completions (server : t) =
  let pool = server.worker_pool in
  let rec pop_all acc =
    Mutex.lock pool.completion_mutex;
    let item =
...
let accept_all (server : t) listener =
  let rec loop () =
    try
...
let poll_loop (server : t) listener wake_read =
  let poller = Poll.create ~maxfds:(2 + server.max_connections) () in
  let progress_connections () =
    let conn_snapshot : connection list =
      Hashtbl.fold (fun _fd (conn : connection) acc -> conn :: acc) server.conns []
    in
    List.iter (fun (conn : connection) ->
...
let add_route (server : t) ~method_ ?(match_type=Exact) pattern handler =
  Mutex.lock server.mutex;
