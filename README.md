...
* **Native ACP Rendering:** * **Thought Stream:** Render agent "reasoning" in a
  chat-like interface.
...
* **Latency:** End-to-end event propagation (Agent -> OCaml -> Swift) must be $< 100\text{ms}$ on standard 5G/Fiber.
* **Privacy:** No third-party servers. All data stays within the user's Tailnet.
* **Efficiency:** The OCaml server should utilize $< 100\text{MB}$ of RAM per idle agent session.
* **Resilience:** The system must handle IP roaming (switching from Wi-Fi to
  LTE) seamlessly via Tailscale's persistent connection logic.
